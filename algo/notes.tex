...
First we must recall the definition of an binary relation
\begin{definition}
    A \textit{binary relation} is relation which we shall denote $\leq$
    (a relation is an operation between two elements of a set $a,b \in S$ returning either true or false)
    where we have three axioms where $\forall a,b,c \in S$
    \begin{itemize}
        \item Reflexivity: $a \leq a$
        \item Anti-Symmetry: if $a \leq b$ and $b \leq a$ then $a = b$
        \item Transitivity: if $a \leq b$ and $b \leq c$ then $a \leq c$
    \end{itemize}
    Consider as an example less than equal on the integers
...
We can prune some branches because of the transitive property of $\leq$,
...
This is another question we have to ask separately.
...
For a three item set $(a,b,c)$ there exists precisely $!3 = 6$ orderings,
